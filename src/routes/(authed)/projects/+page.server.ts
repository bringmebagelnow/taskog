...
export const load: PageServerLoad = async (event) => {
    ...
    if (user.role === "admin") {
        ...
        projectList = await db.select({
            ...
            status: table.project.status,
            deadline: table.project.deadline
        }).from(table.project).innerJoin(table.projectMember, eq(table.project.id, table.projectMember.projectId)).where(eq(table.projectMember.userId, user.id)).orderBy(asc(sql`deadline IS NULL`), asc(table.project.deadline)).limit(10).offset((page * 10) - 10);
        return {
            isAdmin: false,
            projectList
        };
